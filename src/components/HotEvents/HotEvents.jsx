...
const HotEvents = () => {
  const { getHotEvents, bookmarks, toggleBookmark, loading, buttonLoadingStates } = useEvents()
  const navigate = useNavigate()
  ...
  const CustomArrow = ({ onClick, direction }) => (
    <motion.div
      ...
      onClick={onClick}
    >
      {direction === "left" ? <FaArrowLeft className="text-lg" /> : <FaArrowRight className="text-lg" />}
    </motion.div>
  )
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16">
      ...
      {loading ? (
        ...
        >
          <Slider {...settings}>
            {hotEvents.map((event) => (
              <motion.div key={event.id} variants={itemVariants} className="px-3 py-3 group">
                <motion.div
                  whileHover={{
                    y: -8,
                    boxShadow: "0 25px 50px -12px rgba(0, 0, 0, 0.25)",
                  ...
                  <div className="relative overflow-hidden">
                    ...
                    <AnimatePresence mode="wait">
                      <motion.button
                        ...
                        whileHover={{ scale: 1.2 }}
                        whileTap={{ scale: 0.9 }}
                        className={`absolute bottom-3 right-3 p-3 rounded-full bg-white/90 dark:bg-gray-700/90 hover:bg-white dark:hover:bg-gray-700 shadow-lg transition-all ${bookmarks[event.id] ? "text-red-500 shadow-red-500/20" : "text-gray-700 dark:text-gray-300"
                          }`}
                        onClick={(e) => handleLoveClick(e, event.id)}
                        disabled={buttonLoadingStates[`bookmark-${event.id}`]}
                      >
                        {buttonLoadingStates[`bookmark-${event.id}`] ? (
