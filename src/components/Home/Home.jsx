...
// import CategoryCarasoul from "../CategoryCarasoul/CategoryCarasoul"
// import HotEvents from "../HotEvents/HotEvents"
// import EventsCarousel from "../UpcomingEvents/EventsCarousel"
// import { FontAwesomeIcon } from "@fortawesome/react-fontawesome"
// import { faCalendarCheck, faTicketAlt, faUsers, faMapMarkedAlt } from "@fortawesome/free-solid-svg-icons"
...
//   return (
//     <div className="bg-gradient-to-b from-purple-50 to-white dark:from-gray-900 dark:to-gray-800 overflow-hidden">
...
export default function Home() {
  const { dataInitialized } = useEvents()
  const queryClient = useQueryClient()

  // Prefetch data when component mounts if not already initialized
  useEffect(() => {
    if (!dataInitialized && typeof window !== "undefined" && !!localStorage.getItem("token")) {
      // Prefetch events data
      queryClient.prefetchQuery({
        queryKey: ["events"],
      })
    }
  }, [dataInitialized, queryClient])
  ...
  return (
    <div
      className={`${styles.homeContainer}  bg-gradient-to-b from-purple-50 to-white dark:from-gray-900 dark:to-gray-800 overflow-hidden`}
    >
      {/* Decorative background elements */}
      ...
      >
        <motion.div
          variants={itemVariants}
          className={`${styles.featureCard} ${styles.featureCardPurple} bg-gradient-to-br from-purple-500 to-indigo-600 rounded-2xl p-6 text-white shadow-lg transform transition-transform hover:scale-105 hover:shadow-xl`}
        >
          <FontAwesomeIcon icon={faCalendarCheck} className="text-4xl mb-4" />
          <h3 className="text-xl font-bold mb-2">Discover Events</h3>
          <p className="text-purple-100">Find amazing events happening around you and never miss out on the fun.</p>
        ...
        <motion.div
          variants={itemVariants}
          className={`${styles.featureCard} ${styles.featureCardPink} bg-gradient-to-br from-pink-500 to-rose-600 rounded-2xl p-6 text-white shadow-lg transform transition-transform hover:scale-105 hover:shadow-xl`}
        >
          <FontAwesomeIcon icon={faTicketAlt} className="text-4xl mb-4" />
          <h3 className="text-xl font-bold mb-2">Easy Booking</h3>
          <p className="text-pink-100">Book tickets in seconds with our streamlined checkout process.</p>
        ...
        <motion.div
          variants={itemVariants}
          className={`${styles.featureCard} ${styles.featureCardBlue} bg-gradient-to-br from-blue-500 to-cyan-600 rounded-2xl p-6 text-white shadow-lg transform transition-transform hover:scale-105 hover:shadow-xl`}
        >
          <FontAwesomeIcon icon={faUsers} className="text-4xl mb-4" />
          <h3 className="text-xl font-bold mb-2">Connect with Others</h3>
          <p className="text-blue-100">Meet like-minded people who share your interests and passions.</p>
        ...
        <motion.div
          variants={itemVariants}
          className={`${styles.featureCard} ${styles.featureCardAmber} bg-gradient-to-br from-amber-500 to-orange-600 rounded-2xl p-6 text-white shadow-lg transform transition-transform hover:scale-105 hover:shadow-xl`}
        >
          <FontAwesomeIcon icon={faMapMarkedAlt} className="text-4xl mb-4" />
          <h3 className="text-xl font-bold mb-2">Explore Venues</h3>
          <p className="text-amber-100">Discover new and exciting venues for unforgettable experiences.</p>
      ...
      {/* Categories Section */}
      <div
        className={`${styles.categoriesSection} bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-gray-800 dark:to-gray-900 py-8 rounded-t-[3rem] shadow-inner`}
      >
        <CategoryCarasoul />
      </div>

      {/* Hot Events Section */}
      <div className={`${styles.hotEventsSection} bg-white dark:bg-gray-900 py-8`}>
        <HotEvents />
      </div>
    </div>
  )
