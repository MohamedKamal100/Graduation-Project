...
// import { fetchEvents, fetchBookmarks, fetchTickets, addBookmark, removeBookmark, bookTicket } from "../api/eventsApi"
...
//   // Use useMutation to book ticket
//   const bookTicketMutation = useMutation({
//     mutationFn: ({ eventId, quantity }) => bookTicket(eventId, quantity),
//     onSuccess: () => {
//       // Update cached data
...
//   const handleBookTicket = async (eventId, quantity = 1) => {
//     setButtonLoadingStates((prev) => ({
//       ...prev,
//       [`book-${eventId}`]: true,
//     }))
...
//     try {
//       await bookTicketMutation.mutateAsync({ eventId, quantity })
//       return true
//     } catch (error) {
...
//   // دالة للحصول على الفعاليات الساخنة (الأعلى تقييمًا)
...
export const EventsProvider = ({ children }) => {
  ...
  // Function to book a ticket
  const handleBookTicket = async (eventId, quantity = null) => {
    const actualQuantity = quantity || ticketQuantities[eventId] || 1
    const event = getEventById(eventId)
    ...
    try {
      await bookTicketMutation.mutateAsync({ eventId, quantity: actualQuantity })
      toast.success(`Successfully booked ${actualQuantity} ticket${actualQuantity > 1 ? "s" : ""} for "${event.name}"`)
      return true
    } catch (error) {
